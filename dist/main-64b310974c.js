...
function drawEdges(optionNumber,doTransition,transitionType,d){
    ...
    if(!(that.flowScale)&&!(that.flowScale)){
        that.flowScale=d3.scaleLinear()
            .domain([d3.min(edgeFlowList),d3.max(edgeFlowList)])
            .range([2,6]);
        console.log(that.flowScale.domain())
    ...
    //        console.log(edges[i].source+'_'+edges[i].target+':'+edges[i].flowStrokeWidth);
    //    }
    //}
    var markerSize=25;
...
//                    console.log('setFinalOffset');
        ...
        svgEdges.data(edges)
            .attrs({
                d:function(d){return pathData(d,that);},
                transitionStatus:'end'
            })
            .each(function(d){
                var thisEdge=d3.select(this);
...
//                d3.select(this).attr('class','node clicked');
    ...
    if (transitionType == 'flowMap') {
        ...
        for (var i = 0; i < nodes.length; i++) {
            var node = d3.select(svgNodes[0][i]);
            var tmpDelay = [];
            var tmpRatio = [];
            var nodeData;
...
//        if(!nodeData.oriDelay&&!nodeData.oriRatio){
//            nodeData.oriDelay=[];
//            nodeData.oriRatio=[];
//            clone(nodeData.delay,nodeData.oriDelay);
//            clone(nodeData.ratio,nodeData.oriRatio);
//        }
//        else{
//            nodeData.delay=[];
//            nodeData.ratio=[];
//            clone(nodeData.oriDelay,nodeData.delay);
//            clone(nodeData.oriRatio,nodeData.ratio);
//        }
        var r;
...
//                    thisPath.style('stroke',color.yearPathHighLightStroke);
    ...
    for(var i=0;i<dataSet.length-1;i++){
        ...
        var p3=[xScale(dataSet[i+1][0]),yScale(dataSet[i+1][1])];
...
//    console.log(subYearPathDataList);
    ...
    for(var i=0;i<finalSubYearPathDataList.length;i++){
        ...
        if(finalSubYearPathDataList[i].length>=3){
            ...
            subYear.d+='M'+tmp[0][0]+','+tmp[0][1]+'L'+tmp[1][0]+','+tmp[1][1];
            for(var j=2;j<tmp.length;j++){
                subYear.d+=' '+tmp[j][0]+','+tmp[j][1];
            }
        }
...
//        .on('click',yearPathClick)
    for(var i=0;i<dataSet.length;i++){
        if(pathClass!='subYearPath'){
            axisSVG.append('text')
                .attrs({
                    x:xScale(dataSet[i][0])-String(dataSet[i][1]).visualLength()/2,
